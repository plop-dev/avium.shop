...
async function ResetPasswordFormWithToken({ searchParams }: { searchParams: Promise<SearchParams> }) {
	const { token } = await loadSearchParams(searchParams);
	return token ? <ResetPasswordForm token={token}></ResetPasswordForm> : <ForgotPasswordAlert></ForgotPasswordAlert>;
}
						...
						<CardContent>
							<div className='grid gap-6'>
								<Suspense fallback={<p>Loading Form...</p>}>
									<ResetPasswordFormWithToken searchParams={searchParams}></ResetPasswordFormWithToken>
								</Suspense>
